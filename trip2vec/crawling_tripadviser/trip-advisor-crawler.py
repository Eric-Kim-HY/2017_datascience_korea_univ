...
# Modified by smilekhy88@gmail.com 15 May 2017
...
def getreviewpages(domain, activitytype, cityid, activityid, timeout, maxretries, maxreviews, pause, basepath, activity):
    ...
    while True :
        ...
        if htmlpage is None:
            ...
            filename = os.sep.join((path, str(activitypage) + '.html'))
...
activities = ['Attraction', 'Hotel','Restaurant']
...
def main():
    ...
    parser.add_argument(
            ...
            type=int, default=-1)
    parser.add_argument(
            '-o', '--out', default='./',help='Output base path', required=True)
    ...
    with open(os.path.join(args.out, 'ids.txt'), 'w') as file:
        for id in args.ids:
            ...
            for activitylocationid, activityid in sorted(activityids):
                ...
                getreviewpages(domain, args.activity, activitylocationid, activityid, args.timeout, args.maxretries,
                             args.maxreviews, args.pause, basepath, activityid)
                """
